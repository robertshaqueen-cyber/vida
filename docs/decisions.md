...
- 未启用时删除内容中的 ESC，并把 CR/LF 换行折叠为空格。内容仍停留在当前输入行，
  等用户检查后按 Enter；不能为了保留原始换行而直接透传，否则远端 shell 会逐行执行。
...
跨 viewport 选择不能只保存 `(屏幕行, 列)`：滚动后同一屏幕行代表另一段历史，会造成
高亮消失或复制错字。终端帧携带 `viewport_start = history_size - display_offset`，widget
用历史行号保存选区，并只缓存用户实际滚动经过的选区行；滚轮跨屏时继续扩展选区，复制
从缓存历史行拼接。回滚达到上限并淘汰旧行后，不承诺已被 daemon 淘汰内容仍可复制。
...
S2 可选不记住、1/5/15 分钟、1 小时、1 天或 7 天。生产环境的口令只保存在独立
session host 的内存中，带单调时钟过期时间，不进入 Vida 配置、金库文件、日志或系统
Keychain。daemon 重启或 GUI 退出后可重新连接宿主并在有效期内自动解锁；主动锁定、到期、
session host 退出或系统重启都会撤销缓存。宿主在缓存有效时允许无终端会话继续存活，到期后
按原有空闲规则退出。控制协议中的口令字段和 JSON 收发缓冲在 drop 时 zeroize。

此前直接使用 macOS Keychain 的实现会把访问授权绑定到 Cargo 生成的临时 ad-hoc 签名
CDHash；每次重建 daemon 后系统都会把它视为新程序并弹出登录密码确认框。禁止用“允许所有
程序访问”绕过该弹窗，因为那会降低主口令保护边界。正式应用签名与安装包完成前，内存缓存
同时满足 daemon 可替换和无系统弹窗两项要求。
...
shell。宿主发现原父 daemon 已退出、没有控制连接、没有会话且没有有效金库口令缓存时才自行
结束并移除 socket；新 daemon 接管的长控制连接会阻止仍在使用的空宿主竞态退出。
...
### M8 Agent 准备主机草稿，所有者完成凭据与保存

MCP 新增 `host_prepare`，只接受名称、地址、用户名、端口、标签、分组和备注。协议结构使用
`deny_unknown_fields`，因此绕过 MCP 直连 daemon 并夹带密码、私钥或其他未声明字段也会被拒绝。
该工具是人可见写入请求：响应不确定时不得自动重试，避免 GUI 已经收到草稿后重复打开。

daemon 只校验、规范化并审计草稿，审计保存长度与 SHA-256 而不是主机地址或备注明文；随后通过
...
### M10 SFTP 继续复用系统 OpenSSH 与 Owner 边界

SFTP 不引入独立 SSH 协议栈，而由 daemon 使用系统 `sftp` 客户端完成目录读取、新建文件夹、上传和下载。
这样主机地址、端口、密码、私钥文件和内嵌私钥与终端登录使用同一套认证约束；密码或私钥口令
仍经本机一次性 askpass 通道交付，不进入 argv、日志或 GUI。内嵌私钥只写入 Vida 专属 0700
目录中的 0600 临时文件，操作完成立即删除。

文件接口只允许 Owner 身份调用，MCP/Agent 不能列目录或传输文件。GUI 只把用户通过系统文件
选择器确认的本地路径交给 daemon；远端路径作为结构化参数进入固定的 sftp batch 指令，并拒绝
换行、NUL 等控制字符，避免指令注入。目录读取先在远端执行 `cd`，再读取 SFTP 返回的绝对
工作目录，GUI 不猜测 `.` 所代表的家目录。新建文件夹仅接受当前目录下的单个名称，拒绝
斜杠、`.`、`..` 和控制字符。GUI 使用 iced 原生 `FileHovered`/`FileDropped` 接收 macOS 与
Windows 的系统文件拖入事件；每个项目在放下时固定远端目标目录，并经单队列顺序上传，避免
多选拖入时并发启动多个认证和 SFTP 进程。目录上传使用 OpenSSH `put -r`，批量下载同样通过
单队列执行，文件夹使用 OpenSSH 文档规定的 `get -R`。`sftp` 即使单条命令失败也可能以 0 退出，
daemon 因此还会检查 stderr（忽略首次连接的 known-host 提示），禁止把部分创建的空目录报告为成功。当前不实现从远端
拖出到 Finder/资源管理器：iced 只提供文件拖入事件，跨平台拖出还需要系统原生 drag source
和安全的本地暂存生命周期。

目录缓存只存在于 GUI 内存，最多保存 16 个目录并以 30 秒为新鲜期；手动刷新始终读取服务器。
OpenSSH `sftp` 的批处理列表没有可移植的服务端分页接口，因此 daemon 仍一次取得目录元数据，
GUI 则以每批 200 行的方式随滚动增量构造控件。这减少大目录的首屏布局成本，但不虚假声称减少了
首次网络列表开销。此检查点不包含删除、重命名和目录覆盖等破坏性能力。
